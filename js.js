...
$(document).ready(function(){
    ...
    $("body").on('click', '.uitrekenen', function () {
        ...
        // Display awnser in awnser input field.
        $('.antwoord.uitr').val(awnser);
    });
    ...
    $("body").on('click', '.AddOptellenGem', function () {
        ...
        // If the max inputs has been reached "disable" the button.
        if (inputs >= 4) {
    ...
    $("body").on('click', '.gemAntwoord', function () {
        ...
        var thisIs = '';
        let values = [];
        // Set values into array so they can be recalled easier.
        for (i = 1; i <= inputs; i++) {
            // $('body').after('a-');
    ...
    $("body").on('click', '.curAntwoord', function() {
        ...
        // Get the specific rate from the current currency
        .then(cur_rate => cur_rate.json())
        .then(cur_rate => {
            // Get the specific rate from the to be transformed to currency.
            const new_rate = cur_rate.rates[option_two];
            // Calculate the value from a currency to another currency.
            abc = (cur_value.value * new_rate).toFixed(2);
            // Display
            $(conv_value).val(abc + " " + option_two);
        });
    });
    ...
    $("body").on('click', '.Addhoek', function () {
        ...
        if (inputs < 2) {
            $(".selects > .hoeken_" + inputs).after('<p> En </p> <select class="hoeken_2 hoek" size="1"> <option value="">-</option> <option value="a-2">Hoek a</option> <option value="b-2">Hoek b</option> <option value="c-2">Hoek c</option></select>');
        }
        if (inputs >= 1) {
    ...
    $("body").on( 'change', '.hoek', function() {
        ...
        if (inputs < 3) {
            $(".selects").after('<input type="number" placeholder="' + value + '" value="" class="number hoek2 ' + value +'">');
        }
        if (($(hoek_1[1]).val() == "a-2") || ($(hoek_1[1]).val() == "b-2") || ($(hoek_1[1]).val() == "c-2")) {
            console.log("test2");
    ...
    $("body").on('click', '.PytAntwoord2', function () {
        ...
        if ($(hoek_1[0]).val() && $(hoek_1[1]).val()) {
            $(".zijden").css("display", "none");
        }
        // If only one hoek
        this1 = $("#zijdeNumber_1").val();
        this2 = $("#zijdeNumber_2").val();
        if ($("#zijdeNumber_1").val && $("zijdeNumber_2").val) {
            part = Math.cos(this1 / this2);
            awnser = Math.asin(part)*(180/Math.PI);
        }
        if ($(hoek_1[0]).val() && $(hoek_1[1]).val()) {
            awnser = (180 - $(hoek_1[0]).val() - $(hoek_1[1]).val());
        }
        $('.antwoordPyt2').val((awnser).toFixed(2) + "°");
    });
    ...
    /*=======================================================================*/
    $(function(){
        // On number input fields, when certain keys are pcur_ratesed, return false, or return a notification.
        $('body').on("keypcur_rates", '.number', function(e) {
    ...
    /*========================================================================
      Copy Awnser Function
    /*=======================================================================*/
    $("body").on('click', '.CopyAwnser', function() {
        // Get the value of the closest formula then get the associated awnser input field.
        valueA = $(this).closest('.formule').find('.antwoord').val();
        // Copy value of awnser input to user's clipboard.
        navigator.clipboard.writeText(valueA);
    });
});
